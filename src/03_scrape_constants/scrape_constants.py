...
Each scraped value is given the same ASCII symbol name that stage 02's
``translator`` generates for the equations (``\\lambda`` → ``lamda``,
``k_{s}`` → ``k_s``, …), so the constants drop straight into the functions
generated by stage 04.  The generated module also builds a :class:`sympy.Symbol`
for every name, so the constants can be substituted into the symbolic form of
the equations.
...
from translator import latex_to_name, name_to_latex  # noqa: E402
